...
(c) 2025, [Name Redacted for Review], MIT License  

OPTIONS:  

      -a acq        xploit or xplore or adaptor near   = xploit  
...
def mids(data):
  "Return a 'row' holding mid values for x columns; others are '?'."
  row = ["?"] * len(data.cols.all)
  for c in data.cols.x:
    row[c.at] = mid(c)
  return row

def distx(data, r1, r2):
  "Distance between two rows using only x columns."
  d, n = 0, 0
  for c in data.cols.x:
    a, b = r1[c.at], r2[c.at]
    n += 1
    ...
    if a == "?" and b == "?":
      ...
      a = norm(a, c) if a != "?" else "?"
      b = norm(b, c) if b != "?" else "?"

      if a == "?":
        inc = abs(b - (0 if b > 0.5 else 1))
      elif b == "?":
        inc = abs(a - (0 if a > 0.5 else 1))
      else:
        inc = abs(a - b)

    d += inc**the.p

  return (d / (n + 1/BIG)) ** (1/the.p)
...
def like(row, data, nall=100, nh=2):
  ...
  for col in xs:
    ...
    if col.it is Sym:
      p = (col.has.get(v,0) + mprior) / (col.n + m + BIG1)  # <--- use mprior
    else:
      sd  = col.sd + BIG1
      sd2 = sd * sd
      d   = v - col.mu
      p   = exp(-1*(d*d)/(2*sd2)) / ((2*pi*sd2) ** 0.5)
      if p < 0:   p = 0
      elif p > 1: p = 1
...
def actLearn(data, shuffle=True):
  ...
  return o(best=best, rest=rest, todo=todo, evaluated=evaluated)


def nearer(xy, best, rest, todo):
  "Pick and remove one item from todo that is closer to best's mid than rest's mid."
  bmid = mids(best)
  rmid = mids(rest)

  for _ in range(the.Few):
    i   = random.randrange(len(todo))
    row = todo[i]
    if distx(xy, bmid, row) < distx(xy, rmid, row):
      return todo.pop(i)
...
def twoFar(data, rows, sortp=False, above=None, far=0.90):
  """
  Pick two far-apart points a,b from `rows` using x-distance.
  If sortp=True, ensure b is at least as good as a (lower ydist).
  """
  assert rows and len(rows) > 1
  k = int(far * len(rows))
  k = max(1, min(k, len(rows) - 1))

  a = above or sorted(rows, key=lambda r: distx(data, one(rows), r))[k]
  b = sorted(rows, key=lambda r: distx(data, a, r))[k]

  if sortp and ydist(b, data) < ydist(a, data):
    a, b = b, a
  return distx(data, a, b), a, b

def half(data, rows, sortp=False, above=None, far=0.90):
  """
  Split rows into lefts/rights by projecting onto line left<->right.
  Mirrors Lua:
    c,left,right = twoFar(...)
    d = cos(xdist(r,left), xdist(r,right))
    sort by d, split in half
  """
  c, left, right = twoFar(data, rows, sortp=sortp, above=above, far=far)
  denom = 2 * c + 1 / BIG

  def proj(r):
    a = distx(data, r, left)
    b = distx(data, r, right)
    return (a*a + c*c - b*b) / denom

  ordered = sorted(rows, key=proj)
  mid = len(ordered) // 2
  lefts  = ordered[:mid]
  rights = ordered[mid:]

  # last return value in Lua is xdist(left, rights[1]); here: first of rights if exists
  gap = distx(data, left, rights[0]) if rights else 0
  return lefts, left, rights, right, gap

def branch(data, budget=10, rows=None, above=None, sortp=True, far=0.90):
  """
  Recurse down the 'better' half, budget times.
  Mirrors Lua:
    if budget < 1 or #rows < 2: return rows
    lefts,left = half(rows,true,above)
    return branch(budget-1,lefts,left)
  """
  rows = rows or data.rows
  if budget < 1 or len(rows) < 2:
    return rows
  lefts, left, _, _, _ = half(data, rows, sortp=sortp, above=above, far=far)
  return branch(data, budget=budget - 1, rows=lefts, above=left, sortp=sortp, far=far)
#--------- --------- --------- --------- --------- --------- ------- -------
