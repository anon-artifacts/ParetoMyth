...
run_all.lua : contains LINE, peek at a few rows, find a near global best
(c) 2024 [Name Redacted for Review], MIT license.

USAGE:
  lua run_all.lua [OPTIONS] [DEMO]
...
local function split(a,n)--> a,a. Return all before and after `n`th item.
  ...
  return u,v end
  function split_csv(str, delimiter)
    local result = {}
...
function Data:find_row(csv_string)
  local row = split_csv(csv_string, ",")  -- Convert CSV string to a table
  for _, existing_row in pairs(self.rows) do
    local match = true
    for i, value in ipairs(row) do  -- Use ipairs for ordered traversal
      print(existing_row[i])
      if tostring(existing_row[i]) ~= value then
        match = false
        break
      end
    end
...
function Data:dehb(file, budget, rp)
  ...
  local command = 'python3 ../FileResultsReader.py ' ..
                '--data_file_path ' .. filepath .. ' ' ..
                '--folder_name ../../results/results_DEHB/DEHB ' ..
                '--budget ' .. budget
...
function Data:actLearn(file, budget, rp)
  ...
  local command = 'python3 ../FileResultsReader.py ' ..
                '--data_file_path ' .. filepath .. ' ' ..
                '--folder_name ../../results/results_Active_Learning/Active_Learning ' ..
                '--budget ' .. budget
...
function Data:ezrLearn(file, budget, rp)
  -- Define the Python command to execute
  local filepath = file or self.data
  local command = 'python3 ../FileResultsReader.py ' ..
                '--data_file_path ' .. filepath .. ' ' ..
                '--folder_name ../../results/results_EZR/EZR ' ..
                '--budget ' .. budget
...
-- Fast kmeans++: fixed pool + incremental nearest updates
-- function Data:around(budget, rows, z)
--   rows   = rows or self.rows
--   budget = math.min(budget or the.budget, #rows)
--   if budget <= 1 then return { any(rows) } end

--   -- tune this knob: bigger = better quality, slower
--   local poolN = math.min(#rows, (the.samples or 32) * 20)  -- 20x is a good default
--   local pool  = many(rows, poolN)

--   -- start
--   z = { any(rows) }

--   -- nearest distance from each pool point to current centroids
--   local nearest = {}
--   for i = 1, poolN do
--     nearest[i] = fast_xdist(self, pool[i], z[1])
--   end

--   for k = 2, budget do
--     -- total weight = sum(nearest^2)
--     local all = 0
--     for i = 1, poolN do
--       local d = nearest[i]
--       all = all + d*d
...
--       local d = nearest[i]
--       r = r - d*d
--       if r <= 0 then one = pool[i]; break end
--     end
--     z[#z+1] = one

--     -- update nearest distances incrementally (ONE pass)
--     for i = 1, poolN do
--       local d = fast_xdist(self, pool[i], one)
...
function cached_fast_xdist(data, r1, r2)
  local i, j = r1._i, r2._i
  if i > j then i, j = j, i end

  local row = XD_CACHE[i]
  if row then
    local d = row[j]
...
-- Exact-match version of xdist (no re-implemented dist logic)
function fast_xdist(data, r1, r2)
  ...
  -- NOTE: xdist will divide by 0 if n==0; keep same behavior.
  -- If you prefer safety, change this to: if n==0 then return 0 end

  for i = 1, n do
    local col = cols[i]
    local delta = col:dist(r1[col.pos], r2[col.pos])  -- EXACT same dist() used by xdist
    d = d + math.abs(delta) ^ p                       -- matches xdist's math.abs(f(col))^p
  end

  return (d / n) ^ (1 / p)                            -- EXACT same aggregation as xdist
end
...
function _comparez(file,IT)  
  ...
  local TASKS  = {
    ...
    {"ACT-24", function() return data:actLearn(file, 24, Repeats) end}, 
    {"ACT-50", function() return data:actLearn(file, 50, Repeats) end}, 
    {"ACT-100", function() return data:actLearn(file, 100, Repeats) end}, 
    {"ACT-200", function() return data:actLearn(file, 200, Repeats) end},
    {"EZR-6"  , function() return data:ezrLearn(file, 6, Repeats) end},
    {"EZR-12" , function() return data:ezrLearn(file, 12, Repeats) end},
    {"EZR-18" , function() return data:ezrLearn(file, 18, Repeats) end},
    {"EZR-24" , function() return data:ezrLearn(file, 24, Repeats) end},
    {"EZR-50" , function() return data:ezrLearn(file, 50, Repeats) end},
    {"EZR-100", function() return data:ezrLearn(file, 100, Repeats) end},
    {"EZR-200", function() return data:ezrLearn(file, 200, Repeats) end},
  ...
  for _, task in pairs(TASKS) do
    ...
    local is_tabulated = string.match(task[1], "^EZR") or string.match(task[1], "^NSGAIII") or string.match(task[1], "^SMAC") or string.match(task[1], "^ACT") or string.match(task[1], "^DEHB") or string.match(task[1], "^DE") or string.match(task[1], "^TPE")
  ...
  local names = map(TASKS, function(task) 
    return task[1] .. "," .. task[1] .. "_sd," .. task[1] .. "_time"
  end)
  ...
  for _,task in pairs(TASKS) do
     ...
     push(report, fmt("%.0f", 100 * (task[3].sd or 0)))
